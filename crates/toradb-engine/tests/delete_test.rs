...
fn delete(dag: &mut DagRunner, sql: &str) -> usize {
    let stmts = parse(sql).unwrap();
    let toradb_sql::ast::Stmt::Delete { table, where_clause } = &stmts[0] else {
...
fn delete_excludes_from_search_count_and_groupby() {
    ...
    let before = search_ids(&mut dag, "SELECT id FROM docs SPARSE SEARCH body BM25('nikola tesla') LIMIT 10");
    ...
    let after = search_ids(&mut dag, "SELECT id FROM docs SPARSE SEARCH body BM25('nikola tesla') LIMIT 10");
    ...
    let toradb_sql::ast::Stmt::Select(sel) = &stmts[0] else { panic!() };
    let sql_exec::SqlSelectResult::Aggregate(a) = sql_exec::run_select(&mut dag, sel).unwrap() else { panic!() };
    let patent = a.group_keys.iter().zip(&a.value_rows).find(|(k, _)| k.as_str() == "patent").map(|(_, v)| v[0]).unwrap();
...
fn delete_by_id_in_removes_all() {
    ...
    dag.add_documents(
        "docs",
        vec![doc("tesla a", "x"), doc("tesla b", "x"), doc("tesla c", "x"), doc("tesla d", "x")],
    )
    .expect("add");

    assert_eq!(delete(&mut dag, "DELETE FROM docs WHERE id IN (0, 2)"), 2);
    let ids = search_ids(&mut dag, "SELECT id FROM docs SPARSE SEARCH body BM25('tesla') LIMIT 10");
...
fn delete_survives_reopen() {
    ...
    {
        let mut dag = DagRunner::open(&dir).expect("open");
        dag.add_documents("docs", vec![doc("tesla a", "x"), doc("tesla b", "x")]).expect("add");
...
fn compaction_reclaims_tombstoned_rows() {
    ...
    dag.add_documents("docs", vec![doc("tesla a", "x"), doc("tesla b", "x")]).expect("add"); // ids 0,1
    dag.add_documents("docs", vec![doc("tesla c", "x"), doc("tesla d", "x")]).expect("add"); // ids 2,3
    ...
    assert!(tombstones_path.exists(), "tombstone file should exist after delete");
    ...
    assert_eq!(manifest.segment_meta[0].deleted_count, 0, "merged segment has no tombstones");
    ...
    assert!(!tombstones_path.exists(), "tombstone file removed after full reclaim");
