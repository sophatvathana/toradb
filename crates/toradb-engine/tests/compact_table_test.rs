...
fn compact_merge_produces_sorted_output() {
    ...
    let seg_a: Vec<ColumnarDoc> = (0..5u64)
        .map(|i| ColumnarDoc {
            id: i * 2, // 0,2,4,6,8
            text: format!("a-{}", i * 2),
            metadata: Default::default(),
        ...
        .collect();
    let seg_b: Vec<ColumnarDoc> = (0..5u64)
        .map(|i| ColumnarDoc {
            id: i * 2 + 1, // 1,3,5,7,9
            text: format!("b-{}", i * 2 + 1),
            metadata: Default::default(),
    ...
    let manifest =
        TableManifestFile::load(&TableManifestFile::path_for_table(base, table)).unwrap();
    assert_eq!(manifest.segments.len(), 1);
    ...
    for i in 0..ids.len() - 1 {
        assert!(
            ids[i] < ids[i + 1],
            "merged segment is not sorted at position {i}: {} > {}",
            ids[i],
            ids[i + 1]
        );
    }
...
fn compact_promotes_tier0_to_tier1() {
    ...
    let before = TableManifestFile::load(&manifest_path).unwrap();
    assert!(
        !before.segment_meta.is_empty(),
        "segment_meta must be populated on flush"
    );
    assert!(
        before.segment_meta.iter().all(|m| m.tier == 0),
        "all fresh segments are tier 0"
    );
    ...
    assert_eq!(
        after.segment_meta[0].tier, 1,
        "merged segment should be tier 1"
    );
}
...
fn compact_preserves_tier_after_reload() {
    ...
    assert_eq!(
        manifest.segment_meta[0].tier, 1,
        "tier must survive manifest reload"
    );
    assert_eq!(
        manifest.schema_version, 2,
        "schema_version must be bumped to 2"
    );
}
...
fn legacy_db_compacts_without_error() {
    ...
    toradb_storage::columnar::write_segment_with_compression(
        &seg_dir.join(seg1),
        &[doc.clone()],
        None,
        &[],
    ...
    assert_eq!(manifest.segments.len(), 1);
    assert!(
        !manifest.segment_meta.is_empty(),
        "segment_meta must be populated after compact"
    );
}
...
fn fetch_text_correct_with_noncontiguous_sorted_ids() {
    ...
    let docs: Vec<ColumnarDoc> = (0..5u64)
        ...
        .chain((1000..1005u64).map(|i| ColumnarDoc {
            id: i,
            text: format!("high-{i}"),
            metadata: Default::default(),
            embedding: None,
    ...
    let fetched =
        persist::fetch_documents_by_ids(&store, Some(base), table, &all_ids, None).unwrap();
    assert_eq!(fetched.len(), 10, "all 10 docs must be found");

    let by_id: std::collections::HashMap<u64, String> = fetched
        .into_iter()
        .map(|(id, doc)| (id, doc.text))
        .collect();
    for i in 0..5u64 {
        assert_eq!(
            by_id.get(&i).map(|s| s.as_str()),
            Some(format!("low-{i}").as_str()),
            "wrong text for id={i}"
        );
    }
...
fn compact_text_survives_gap_ids() {
    ...
    let store = toradb_index::CorpusStore::default();
    let fetched =
        persist::fetch_documents_by_ids(&store, Some(base), "docs", &all_ids, None).unwrap();
    assert_eq!(fetched.len(), 6, "all 6 docs must be found");
    for (id, doc) in &fetched {
        assert!(
            !doc.text.is_empty(),
            "doc {id} has empty text after compaction"
        );
    }

    let by_id: std::collections::HashMap<u64, &str> = fetched
        .iter()
        .map(|(id, doc)| (*id, doc.text.as_str()))
        .collect();
...
fn compact_segment_bounds_correct_after_merge() {
    ...
    assert!(
        ...
        "segment_id_ranges must be populated after compaction"
    );
    ...
    for range in &manifest.segment_id_ranges {
        ...
        let actual_max = docs.iter().map(|d| d.id).max().unwrap();
        assert_eq!(
            range.min_id, actual_min,
            "min_id mismatch for {}",
            range.file
        );
        assert_eq!(
            range.max_id, actual_max,
            "max_id mismatch for {}",
            range.file
        );
    }
...
fn compact_sql_query_text_nonempty() {
    ...
    for i in 0..10 {
        dag.add_documents(
            "docs",
            vec![make_doc(&format!("the quick brown fox document {i}"))],
        )
        .unwrap();
    }
    ...
    let stmts = parse(
        "SELECT id, text, score FROM docs SPARSE SEARCH body BM25('quick brown fox') LIMIT 10",
    )
    .unwrap();
    ...
    assert!(
        !result.ids.is_empty(),
        "search should return results after compaction"
    );
    ...
    if let Some((_, SqlProjectedColumn::Str(texts))) = text_col {
        for (i, text) in texts.iter().enumerate() {
            ...
            assert!(
                !text.is_empty(),
                "doc {id} has empty text in SQL result after compaction"
            );
        }
    }
}
