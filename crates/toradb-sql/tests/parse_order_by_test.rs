...
fn parses_order_by_score_desc() {
    ...
    assert_eq!(
        sel.order_by,
        Some(OrderBy { column: "score".into(), descending: true })
    );
...
fn parses_order_by_score_asc() {
    assert_eq!(
        ...
        Some(OrderBy { column: "score".into(), descending: false })
    );
}
...
fn parses_order_by_metadata_column_defaults_to_asc() {
    assert_eq!(
        ...
        Some(OrderBy { column: "published".into(), descending: false })
    );
}
...
fn parses_order_by_metadata_column_desc() {
    assert_eq!(
        order_by("SELECT id, published FROM docs SPARSE SEARCH body BM25('x') ORDER BY published DESC"),
        Some(OrderBy { column: "published".into(), descending: true })
    );
}
